...
class Data():
    def __init__(self, data_path, percent) -> None:
        ...
        indices = [torch.LongTensor(x).T for x in indices]  #value : [2, 491]
        values = [torch.FloatTensor(x) for x in values]   # vlue: 491
...
class GraphRule:
    def __init__(self, args, dataset):
        ...
        self.entity_num, self.rel_num = self.data.getinfo() #2378 12
        self.id2e = self.data.id2e #2378
        self.id2r = self.data.id2r #25
        self.nx = self.data.nx #h:{t:[r]}
        self.topK = 3
        ...
        while sam < 1000:
            ...
            for __ in range(self.rule_len): # 3
                ...
                if node[-1] in self.nx[node[0]]:
                    tmp = self.nx[node[0]][node[-1]]  #如果为第一次的话,应该是h1,r1,t1, 
                else:
                    continue
    ...
    def getNewTriple(self, mat, truthSet, owl):
        ...
        pre = int(rightCnt)/len(newTriples) if len(newTriples) else 0
        recall = sqrt(int(rightCnt) / len(self.data.htPair[truthSet]))
        f1 = 2 / (len(newTriples)/int(rightCnt) + sqrt(len(self.data.htPair[truthSet])/int(rightCnt)))
        mrr = sumrank
        message = "{"
        message += '"split":"{}", "owl":"{}", "label":"J{}-{}-{}", "add":{}, "true":{},  "MRR":{}, "len_test":{}, "pre":{}, "recall":{}, "f1":{}'.format(
                truthSet, owl, self.args.rule_len, self.args.hc_thr, self.args.sc_thr,
                len(newTriples), int(rightCnt), mrr, len(self.data.htPair[truthSet]), pre, recall, f1)
        message += "}\n"
...
if __name__ == '__main__':
    ...
    parser.add_argument('-rule_len', type=int, default=3)
    parser.add_argument('-sc_thr', type=float, default=0.05)
    parser.add_argument('-hc_thr', type=float, default=0.85)
