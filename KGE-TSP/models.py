...
class KGEModel(nn.Module, ABC):
    ...
    def test_step(model, data_reader, mode, args, steps):
        ...
        for owl in [True]:    
            for test in ['VALID', 'TEST']:
                ...
                trueCnt, addedTrueCnt, idx, rank_idx = 0, 0, 0, 0
                Len =  len(scores)
                for thr in sorted(thrs, reverse=True):
                    ...
                    with open(f"{args.save_path}/../metric-SP{args.perfix}-data{args.data}-eval{args.best_evaluate}.txt", 'a') as me:
                        me.write('"save_path:{}"'.format(args.save_path))
                        me.write("\n{")
                        me.write('"split":"{}", "owl":"{}", "label":"{}-{}-{}",  "len_test":{}, "F1":{}, "add":{}, "true":{}, "rank":{}, "pre":{},"recall":{}'.format(
    ...
    def GNNtest(self, model, data_reader, ht_scos, args, steps):
        ...
        if args.thr < 0:
            if args.model ==  "HAKE":
                thrs = [10, 3, 1, 0.5, 0.35, 0.3, 0.2, 0.1, 0.05, 0.01]
            if args.model == "PairRE":
                thrs = [10, 3, 1, 0.5, 0.35, 0.3, 0.2, 0.1, 0.05, 0.01, 0.0035, 0.001, 0.005, 0.00035, 0.0001]
            ...
            for test in ['VALID', 'TEST']:
                ...
                for thr in sorted(thrs, reverse=True):
                    ...
                    if test == 'VALID' and args.best_evaluate == 'F1':
                        ...
                        if metrics['MRR'] > bestMRR: bestMRR = metrics['MRR']
                    with open(f"{args.save_path}/../{args.bs}-GNN_metric-SP{args.perfix}-dataset-{args.data}-eval{args.best_evaluate}.txt", 'a') as me:
                        me.write('"save_path:{}"'.format(args.save_path))
                        me.write("\n{")
