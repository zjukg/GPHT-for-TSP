...
class DataReader(object):
    def __init__(self, data_path: str, prefix: str = ""):
        entity_dict_path = os.path.join(data_path, 'entities.dict')
        relation_dict_path = os.path.join(data_path, 'relations.dict')
        train_data_path = os.path.join(data_path, prefix + 'train.txt')
        valid_data_path = os.path.join(data_path, prefix + 'valid.txt')
        test_data_path = os.path.join(data_path, prefix + 'test.txt')
    ...
    def __getitem__(self, idx):
        ...
        if self.batch_type == BatchType.HEAD_BATCH:
            tmp = [(0, rand_head) if (rand_head, relation, tail) not in self.triple_set
                   else (-2, head) for rand_head in range(self.num_entity)]
            tmp[head] = (0, head)
        elif self.batch_type == BatchType.TAIL_BATCH:
            tmp = [(0, rand_tail) if (head, relation, rand_tail) not in self.triple_set
                   else (-2, tail) for rand_tail in range(self.num_entity)]
            tmp[tail] = (0, tail)
        else:
