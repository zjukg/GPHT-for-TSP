...
from model_open1 import HAKE,  PairRE
...
def parse_args(args=None):
    ...
    parser.add_argument('-data', type=str, default='wiki_select_new_3')
    ...
    parser.add_argument('--bs', default="atte", type=str)
    ...
    parser.add_argument('--max_steps', default=800000, type=int)
    ...
    parser.add_argument('--init_steps', default=600000, type=int)
...
def main(args):
    ...
    r_dict_1 = data_pretrain(args.data, 0.8)
    ...
    if args.testGNN:
        ...
        if not args.do_train:
            kge_model.GNNtest(kge_model, data_reader, ht_scos, args, step, r_dict_1)
            exit()
    ...
    if args.do_train:
        ...
        for step in range(init_step, args.max_steps+1):
            ...
            if args.do_test and step % args.valid_steps == 0 and step >= init_step + args.valid_steps and step>=args.init_steps:
            #if args.do_test and step % args.valid_steps == 0 and step > init_step + args.valid_steps  and step>=20000:
                #args.valid_steps *= 2
                print('\r', end='')
                logging.info('Evaluating on Test Dataset...')
                if args.testGNN:
                    metrics = kge_model.GNNtest(kge_model, data_reader, ht_scos, args, step, r_dict_1)
                else:
                    metrics = kge_model.test_step(kge_model, data_reader, ModeType.TEST, args, step, r_dict_1)
                start_time = time.time()
    ...
    if args.do_valid:
        ...
        metrics = kge_model.test_step(kge_model, data_reader, ModeType.VALID, args, step, r_dict_1)
